...
{
    return ParallelFor( v.beginId(), v.endId(), [&] ( size_t i )
    {
        std::forward<F>( f )( I( i ) );
    } );
}
...
{
    return Parallel::For( begin, end, Parallel::CallSimplyMaker{}, std::forward<F>( f ), std::forward<Args>( args )... );
}
...
{
    return Parallel::For( begin, end, Parallel::CallWithTLSMaker<L>{ e }, std::forward<F>( f ), std::forward<Args>( args )... );
}
